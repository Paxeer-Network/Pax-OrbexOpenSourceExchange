...
class ExchangeManager {
  ...
  private async initializeExchange(
    ...
    try {
      ...
      try {
        credentialsValid = await exchange.checkRequiredCredentials();
        console.log(`Credential validation for ${provider}:`, credentialsValid);
      } catch (credError) {
        console.log(`Credential check failed for ${provider}, but trying to proceed:`, credError.message);
        // Don't fail here - some exchanges have issues with checkRequiredCredentials
        // but still work for authenticated endpoints
        credentialsValid = true;
      }
      
      if (!credentialsValid) {
        console.log(`Creating unauthenticated exchange for ${provider}`);
        logError(
          "exchange",
        ...
        console.log(`Using authenticated exchange for ${provider}`);
      }

      try {
        await exchange.loadMarkets();
        console.log(`Successfully loaded markets for ${provider}`);
      } catch (error) {
        if (this.isRateLimitError(error)) {
          await this.handleRateLimitError(provider);
          return this.initializeExchange(provider, retries);
        } else {
          console.log(`Failed to load markets for ${provider}, but keeping authenticated exchange:`, error.message);
          logError(
            "exchange",
            new Error(`Failed to load markets: ${error.message}`),
            __filename
          );
          // Don't close the authenticated exchange - loadMarkets might fail but other endpoints could work
          // await exchange.close();
          // exchange = new ccxt.pro[provider]();
        }
      }
