...
export async function getFiltered({
  ...
  if (sortField.includes(".")) {
    ...
    if (association) {
      // Verify the attribute exists in the associated model
      const associatedModel = association.target;
      const associatedModelAttributes = Object.keys(associatedModel.rawAttributes);
      
      if (associatedModelAttributes.includes(attribute)) {
        order = [
          [{ model: association, as: relation }, attribute, validSortOrder],
        ];
      } else {
        console.error(`Attribute '${attribute}' not found in model '${relation}'. Available attributes:`, associatedModelAttributes);
        // Fallback to default sorting
        order = [["createdAt", validSortOrder]];
      }
    } else {
      console.error(`Association '${relation}' not found for model. Available associations:`, Object.keys(model.associations));
      // Fallback to default sorting
      order = [["createdAt", validSortOrder]];
    }
  } else {
