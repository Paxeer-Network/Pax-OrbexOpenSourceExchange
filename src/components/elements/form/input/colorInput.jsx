...
const ColorInput = ({ label, addon, size = "md", color = "default", shape = "smooth", error, loading = false, icon, className: classes = "", noPadding = false, setFirstErrorInputRef, warning, ...props }) => {
    ...
    return (<div className="w-full">
      ...
      <div className={`relative w-full ${addon ? "flex" : ""}`}>
        ...
        <input type="color" ref={inputRef} className={(0, input_variants_1.colorInputVariants)({
            size,
            color,
            shape,
            className: `peer 
        ${classes}
        ${size === "sm" && icon ? "pe-1 ps-8" : ""}
        ${size === "md" && icon ? "pe-2 ps-10" : ""}
        ${size === "lg" && icon ? "pe-3 ps-12" : ""}
        ${size === "sm" && !icon ? "px-1" : ""}
        ${size === "md" && !icon ? "px-2" : ""}
        ${size === "lg" && !icon ? "px-3" : ""}
