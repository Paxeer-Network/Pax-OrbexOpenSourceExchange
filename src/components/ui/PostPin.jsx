...
const PinContainer = ({ children, title, href, className, containerClassName, }) => {
    ...
    return (<div className={(0, cn_1.cn)("relative group/pin z-50  cursor-pointer", containerClassName)} onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
      <div style={{
            perspective: "1000px",
        ...
        <div style={{
            transform: transform,
        }} className="absolute left-1/2 p-4 top-1/2 bg-white dark:bg-muted-900 flex justify-start items-start  rounded-2xl  shadow-[0_8px_16px_rgb(0_0_0/0.4)] border border-white/[0.1] group-hover/pin:border-white/[0.2] transition duration-700 overflow-hidden">
          <div className={(0, cn_1.cn)(" relative z-50 ", className)}>{children}</div>
        </div>
      </div>
...
const PinPerspective = ({ title, href, }) => {
    return (<framer_motion_1.motion.div className="pointer-events-none  w-96 h-80 flex items-center justify-center opacity-0 group-hover/pin:opacity-100 z-60 transition duration-500">
      <div className=" w-full h-full -mt-7 flex-none  inset-0">
        ...
        <>
          <framer_motion_1.motion.div className="absolute right-1/2 bottom-1/2 bg-linear-to-b from-transparent to-cyan-500 translate-y-[14px] w-px h-20 group-hover/pin:h-40 blur-[2px]"/>
          <framer_motion_1.motion.div className="absolute right-1/2 bottom-1/2 bg-linear-to-b from-transparent to-cyan-500 translate-y-[14px] w-px h-20 group-hover/pin:h-40  "/>
