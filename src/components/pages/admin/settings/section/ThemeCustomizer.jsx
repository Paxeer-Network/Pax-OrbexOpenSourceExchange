...
const colorInput_1 = __importDefault(require("@/components/elements/form/input/colorInput"));
const themeFields = {
    primary: [
        "50",
        "100",
...
const ThemeCustomizer = ({ formData, handleInputChange, handleCancel, handleSave, hasChanges, isLoading, }) => {
    const { t } = (0, next_i18next_1.useTranslation)();
    const [colorValues, setColorValues] = (0, react_1.useState)({});
    const [categoryChanges, setCategoryChanges] = (0, react_1.useState)({});
    (0, react_1.useEffect)(() => {
        if (formData.themeColors) {
            setColorValues(formData.themeColors);
        }
        else if (typeof window !== "undefined") {
            const initialColorValues = Object.entries(themeFields).reduce((acc, [color, shades]) => {
                acc[color] = {};
    ...
    return (<div className="mt-4 grid w-full grid-cols-12 gap-6">
      ...
      <div className="col-span-12 lg:col-span-8 ltablet:col-span-8">
        <div className="lg:max-w-xl space-y-6">
          {Object.entries(themeFields).map(([color, shades]) => (<div key={color} className="space-y-4">
              ...
              <div className="grid w-full grid-cols-12 gap-x-6 gap-y-4">
                {shades.map((shade) => {
                var _a;
                const value = ((_a = colorValues[color]) === null || _a === void 0 ? void 0 : _a[shade]) || "#ffffff";
                return (<div key={shade} className="col-span-12 md:col-span-6 lg:col-span-4 flex items-center space-x-4">
                      <colorInput_1.default label={`${color}-${shade}`} type="color" value={value} onChange={(e) => handleChange(color, shade, e.target.value)} noPadding icon="fluent:color-20-filled"/>
                    </div>);
            })}
              </div>
              {categoryChanges[color] && (<div className="flex justify-end space-x-4">
                  <Button_1.default color="default" onClick={() => handleReset(color)}>
                    {t("Reset")}
                  </Button_1.default>
                  <Button_1.default color="primary" onClick={() => saveCategoryChanges(color)} loading={isLoading}>
                    {t("Save Changes")}
                  </Button_1.default>
                </div>)}
            </div>))}
...
exports.default = ThemeCustomizer;
